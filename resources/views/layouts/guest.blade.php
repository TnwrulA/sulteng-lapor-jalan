...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="font-sans text-[#1a1e1b] antialiased bg-[#fafafa]">
        <div x-data="globalGuestHandler()" x-init="init()" class="relative min-h-screen app-surface flex flex-col items-center justify-center pt-6 sm:pt-0 px-4">
            ...
            <div class="mb-4">
                <a href="/" class="flex flex-col items-center gap-2 group">
                    <span class="flex h-14 w-14 items-center justify-center rounded-2xl bg-gradient-to-br from-[#ff5e00] to-[#ff7e33] text-white shadow-lg shadow-[#ff5e00]/15 transition-all duration-300 group-hover:scale-105">
                        <x-application-logo class="h-8 w-8" />
                    </span>
                    <span class="text-sm font-extrabold text-[#1a1e1b] tracking-wider uppercase mt-1">Sulteng Lapor Jalan</span>
                </a>
            </div>

            <!-- Auth Form Card Wrapper -->
            <div class="glass-card w-full p-6 sm:max-w-md border border-[#ff5e00]/10 shadow-lg">
                {{ $slot }}
            </div>
        </div>

        <script>
            function globalGuestHandler() {
                return {
                    isLoading: false,
                    loadingText: 'Memuat...',
                    toasts: [],
                    init() {
                        ...
                        document.addEventListener('submit', (e) => {
                            ...
                            if (!form.dataset.submitting) {
                                e.preventDefault();
                                window.showLoading(form.dataset.loadingText || 'Menyinkronkan data...');
